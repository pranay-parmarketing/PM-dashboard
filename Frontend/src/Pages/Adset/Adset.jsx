...
const Adset = () => {
  ...
  const totalPages = Math.ceil(mongoData.length / rowsPerPage);
    ...
    const fetchCampaigns = async () => {
      try {
        const response = await axios.get(
          `${MONGO_URI}/api/save-adset/${selectedAccount.id}`
        );
  ...
  return (
    <div className="home">
      <ChooseFileModal
        ...
        errorMessage={error}
        apiEndpoint={`${MONGO_URI}/api/save-adset`}
      />
      <AddModal
        Name={"Adset"}
      ...
      <EditModal
        ...
        endpoint={`${MONGO_URI}/api/save-adset/${selectedRow?._id}`} // Use dynamic endpoint with user ID
        onSuccess={() => {
                  ...
                  filteredData
                    ...
                    .map((row, index) => (
                      <tr key={row.id}>
                        ...
                        <td data-label="Account">{row?.campaign_name}</td>
