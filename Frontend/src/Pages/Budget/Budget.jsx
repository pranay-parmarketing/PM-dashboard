...
//         } else {
//           const url = `https://graph.facebook.com/v17.0/${id}?fields=id,name,created_time,adsets{daily_budget,lifetime_budget,budget_remaining,status,insights{spend},campaign{id,name},ads{id,name}}&access_token=${accessToken}&appsecret_proof=${appSecretProof}`;
...
//                           {console.log(
//                             detail.adsets?.data?.[0]?.budget_remaining
//                           )}
//                           {/* Provide a fallback value if data is undefined */}
//                           {detail.adsets?.data?.[0]?.daily_budget/100 ||
//                             "Data not available"}
//                         </td>
...
//       } else {
//         // Campaign API URL
//         const campaignUrl = `https://graph.facebook.com/v17.0/${id}?fields=id,name,account_id,status,daily_budget&access_token=${accessToken}&appsecret_proof=${appSecretProof}`;
//         const campaignResponse = await fetch(campaignUrl);
//         const campaignResult = await campaignResponse.json();

//         let budgetDetails = {};

//         if (campaignResult && campaignResult.daily_budget) {
//           // If campaign has a daily_budget, use it
//           budgetDetails = {
//             type: "Campaign",
//             budget: campaignResult.daily_budget,
//           };
//         } else {
//           // If no campaign budget, fetch adset budgets
//           const adsetsUrl = `https://graph.facebook.com/v17.0/${id}/adsets?fields=id,name,status,daily_budget,budget_remaining&access_token=${accessToken}&appsecret_proof=${appSecretProof}`;
...
//             adsetsResult &&
//             adsetsResult.data &&
//             adsetsResult.data.length > 0
//           ) {
//             // Calculate the total budget from adsets
//             const totalBudget = adsetsResult.data.reduce(
//               (sum, adset) => sum + (parseInt(adset.daily_budget) || 0),
//               0
//             );
...
//               type: "Adsets",
//               budget: totalBudget,
//               adsets: adsetsResult.data, // Optionally, keep all adset details
//             };
//           }
//         }
...
//                         </td>
//                         <td data-label="Total Budget" className="px-4 py-2">
//                           {/* Check if campaign has a daily budget */}
//                           {detail.daily_budget
//                             ? parseInt(detail.daily_budget) / 100 // Use the campaign's daily budget (convert from cents if needed)
//                             : detail.adsets?.data?.length > 0 // Check if adsets exist
//                             ? detail.adsets.data.reduce(
//                                 (sum, adset) =>
//                                   sum + (parseInt(adset.daily_budget) || 0), // Sum up adsets' daily budgets
//                                 0
//                               ) / 100 // Convert from cents to dollars if necessary
//                             : "Data not available"}{" "}
...
//         // Check if the cached data exists and if it was fetched within the last 24 hours
//         if (
//           cachedData &&
//           cacheTimestamp &&
//           Date.now() - cacheTimestamp < 86400000 // 24 hours in milliseconds
//         ) {
//           return JSON.parse(cachedData);
//         } else {
//           const url = `https://graph.facebook.com/v17.0/${id}?fields=id,name,created_time,adsets{daily_budget,lifetime_budget,budget_remaining,status,insights{spend},campaign{id,name},ads{id,name}}&access_token=${accessToken}&appsecret_proof=${appSecretProof}`;
                        ...
                        <td data-label="Total Budget" className="px-4 py-2">
                          ...
                          detail.adsets.data.length > 0
                            ? detail.adsets.data.reduce((total, adset) => {
                                const budget =
                                  parseInt(adset.daily_budget, 10) || 0; // Parse daily_budget to integer
                                return total + budget;
                              }, 0) / 100 // Divide by 100 if needed (e.g., to convert cents to dollars)
                            : "Data not available"} */}

                          {/* {console.log('detail',detail.adsets.data[0].campaign.daily_budget)} */}

                          {/* {detail.adsets.data[0].campaign.daily_budget
                            ? parseInt(detail.adsets.data[0].campaign.daily_budget, 10) /
                              100 // Divide by 100 if needed
                            : Array.isArray(detail?.adsets?.data) &&
                              detail?.adsets?.data.length > 0
                            ? detail?.adsets?.data.reduce((total, adset) => {
                                const budget =
                          ...
                          {/* {
  detail.budgetDetails?.type === "Campaign" && detail.budgetDetails?.budget
    ? detail.budgetDetails.budget / 100
    : Array.isArray(detail.adsets) && detail.adsets.length > 0
    ? detail.adsets.reduce(
        (total, adset) =>
          total + (parseInt(adset.daily_budget?.["$numberInt"]) || 0),
        0
      ) / 100
    : "Data not available"
} */}
