...
const createExpenseData = async (req, res) => {
  try {
    ...
    if (!Array.isArray(budgets)) {
      return res
        .status(400)
        .json({ error: "Invalid data format. Expected an array of budgets." });
    }
    ...
    for (const budget of budgets) {
      ...
      const processedAdsets =
        budget.adsets?.data?.map((adset) => ({
          ...
          insights: {
            data:
              adset.insights?.data?.map((insight) => ({
          ...
          campaign: {
            ...
            name: adset.campaign?.name || "Unknown Campaign",
            daily_budget:
              adset.campaign?.daily_budget || "Unknown daily_budget",
          },
          ads: {
            data:
